...
void PublishMountControl(){
  ...
  static int roll = 0;
  ROS_INFO("send mount control");
  msg.header.stamp = ros::Time::now();
  msg.header.frame_id = "map";
  msg.mode = 2;
  msg.pitch = 1 / 3.14 * 180; // Gimbal Pitch
  msg.roll = roll / 3.14; // Gimbal  Yaw
  msg.yaw = 1 / 3.14 * 180; // Gimbal  Yaw
  roll ++;
  if(roll == 360){
    roll = 0;
  }
...
void PublishMountConfigure(){
  mavros_msgs::CommandLong msg;
  int banana = 0;
  // a request is sending a command long
  // a response if the ACK ??
  msg.request.broadcast = false;
}
...
int main(int argc, char **argv)
{
    ...
    mount_control_pub_ = nh.advertise<mavros_msgs::MountControl>("/mavros/mount_control/command", 1);
